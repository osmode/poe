...
public:
	...
	//reverse-lookup: given value=string 's', find its key
	//returns -1 if no such value in hash table
	int findValue(string s) const;

	//counter is the value given to each word in the hash
	//table and is incremented with each new entry
	static int counter;

	//overload [] operator to return 'data' in  hashArray[]
